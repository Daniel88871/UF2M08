...
public class HomeFragment extends Fragment {
    ...
    class PostsAdapter extends FirestoreRecyclerAdapter<Post, PostsAdapter.PostViewHolder> {
        ...
        public PostViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            return new PostViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.fragment_post, parent, false));
        }

        @Override
        protected void onBindViewHolder(@NonNull PostViewHolder holder, int position, @NonNull final Post post) {
            Glide.with(getContext()).load(post.authorPhotoUrl).circleCrop().into(holder.PhotoImageView);
            ...
            holder.retweetImageView.setOnClickListener(view -> {
                FirebaseFirestore.getInstance().collection("posts").document(postKey).update("retweets." + uid, post.likes.containsKey(uid) ? FieldValue.delete() : true);
            });
        ...
        class PostViewHolder extends RecyclerView.ViewHolder {
            ImageView PhotoImageView, likeImageView, retweetImageView, mediaImageView;
